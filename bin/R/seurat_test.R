...
clust.res(seurat.obj = seurat_data_integrated.sexscale, by = 0.1)
graphics.off()
...
##### run pipeline on just new data

# make dataframe with tissue matching directory
sample = c("THI725A1", "THI725A2", "THI725A3", "THI725A4")
matches <- sapply(sample, function(x) file.path[grep(pattern = x, x = file.path)])
sample.paths <- data.frame(sample = names(matches), path = matches, row.names = NULL)
...
# The four Seurat objects are then merged, before running CreateSeuratObject again on the output in order to apply the min.cells parameter on the final merged dataset.
seurat_data <- merge(THI725A1, y = c(THI725A2, THI725A3, THI725A4), add.cell.ids = c("hh4-1", "ss4-1", "ss8-1", "hh6-1", "hh5-2", "hh6-2", "hh7-2", "ss4-2"), project = "chick.10x")
seurat_data <- CreateSeuratObject(GetAssayData(seurat_data), min.cells = 3, project = "chick.10x.mincells3")

# The original Seurat objects are then removed from the global environment
rm(THI725A1, THI725A2, THI725A3, THI725A4, sample.paths)
...
# Save RDS after scaling as this step takes time
saveRDS(norm.data, paste0(rds.path, "norm.data.new.RDS"))
#####################################################################################################
#                    Perform dimensionality reduction by PCA and UMAP embedding                    #
#####################################################################################################
...
# Change plot path
curr.plot.path <- paste0(plot.path, '0_filt_data_new/')
dir.create(curr.plot.path)
...
# Change plot path
curr.plot.path <- paste0(plot.path, '1_sex_filt_new/')
dir.create(curr.plot.path)
...
cell.sex.ID <- list("male.cells" = k.male, "female.cells" = k.female)
saveRDS(cell.sex.ID, paste0(rds.path, "sex_kmeans_new.RDS"))
...
# Save RDS
saveRDS(norm.data.sexscale, paste0(rds.path, "norm.data.sexscale_new.RDS"))
...
# Change plot path
curr.plot.path <- paste0(plot.path, "2_contamination_filt_new/")
dir.create(curr.plot.path)
