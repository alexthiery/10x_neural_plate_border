...
sctransform_data <- lapply(seurat_split, function(x) SCTransform(x, method = "glmGamPoi", verbose = TRUE, vars.to.regress = "percent.mt"))

# Find and filter integration anchors
features <- SelectIntegrationFeatures(sctransform_data, nfeatures = 2000)
...
# Run seurat integration with rpca
sctransform_data <- FindIntegrationAnchors(object.list = sctransform_data, normalization.method = "SCT", anchor.features = features, dims = 1:30, reduction = "rpca", k.anchor = 5)
