...
# Set paths and load data
{
  if(length(commandArgs(trailingOnly = TRUE)) == 0){
    ...
    plot_path = "./output/NF-downstream_analysis_stacas/filtered_seurat/antler/gene_modules/plots/"
    rds_path = "./output/NF-downstream_analysis_stacas/filtered_seurat/antler/gene_modules/rds_files/"
    gm_path = "./output/NF-downstream_analysis_stacas/filtered_seurat/antler/gene_modules/gene_module_lists/"
    antler_path = "./output/NF-downstream_analysis_stacas/filtered_seurat/antler/gene_modules/antler_data/"
...
metadata = c("stage", "scHelper_cell_type", "run")
...
if(length(unique(seurat_data$run)) > 1){
  ...
  custom_order <- c(NA, "early_non_neural", "non_neural", "early_PPR", "pPPR", "aNPB",
                    "early_border", "neural_progenitors",
                    "a_neural_progenitors", "early_forebrain", "forebrain", "early_midbrain",
                    "midbrain", "early_hindbrain", "hindbrain", "NC", "delaminating_NC")
  
  seurat_data@meta.data$scHelper_cell_type <- factor(seurat_data@meta.data$scHelper_cell_type, levels = custom_order)
  
  gms <- antler_data$gene_modules$lists$unbiasedGMs$content[
    c(50, 48, 51, 49, 45, 46, 47, 24, 20, 21, 22, 23, 19, 18)]
  
  
  # gms <- antler_data$gene_modules$lists$unbiasedGMs_DE_batchfilt$content[
  #   c("GM50", "GM48", "GM49", "GM51", "GM45", "GM47", "GM24", "GM20", "GM21", "GM22", "GM23", "GM19", "GM18")]
  # 
  ncell = ncol(seurat_data)
