...
clust.res <- function(seurat.obj, by = 0.1, starting_res = 0, prefix = "RNA_snn_res."){
  plots <- list()
  ...
  seurat.obj@meta.data <- seurat.obj@meta.data[,!grepl(prefix, colnames(seurat.obj@meta.data))]
  seurat.obj <- FindClusters(seurat.obj, resolution = resolutions, verbose = F)
  plots[["clustree"]] <- clustree(seurat.obj@meta.data, prefix = prefix)
  for(res in resolutions[2:length(resolutions)]){
    plots[[paste(res)]] <- DimPlot(seurat.obj, group.by =  paste0(prefix, res)) +
